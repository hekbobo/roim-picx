<template>
	<div class="w-full bg-rose-100 rounded-md shadow-sm overflow-hidden relative">
		...
		<el-icon class="flex items-center justify-center w-full h-40 lg:h-60">
			<font-awesome-icon :icon="faCopy" class="w-1/2 h-1/4 text-gray-600" />
		</el-icon>
		...
		<div class="w-full absolute left-0 bottom-0 bg-slate-800/70 backdrop-blur-sm">
			...
			<div v-if="mode === 'uploaded'">
				...
				<div class="w-full flex text-white h-9 text-center text-sm">
					...
					<el-popconfirm title="确认删除文件吗？" confirm-button-type="danger" @confirm="() => {
						// (e: Event) => boolean ???
						// loading = true
						emit('delete')
						return true
...
import { faTimesCircle, faTrashAlt, faCopy, faFile } from '@fortawesome/free-regular-svg-icons'
import copy from 'copy-to-clipboard'
import formatBytes from '../utils/format-bytes'
import { ElTooltip, ElDivider, ElPopconfirm, ElImage, ElMessage, ElCard } from 'element-plus'
import { ref } from 'vue'
